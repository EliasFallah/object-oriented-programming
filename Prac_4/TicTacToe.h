...
{ 										// Where to get the board data?      confusing question
	cout << "   1    2    3" << endl	// It looks at the value of board[i][j] each time i and j are iterated through the for loop
		 << endl;
	for (int i = 0; i < 3; i++)
	{
		cout << i + 1;
...
{ // What does & mean?
	// Calls the reference argument, the addresses of x and y are passed into the function allowing them to be modified within the function.
	if (noOfMoves >= 9)
		return false;
...
{ // What is the counterpart of this function in the original code
  //  int main()
	// seed a new set of random numbers at the start of the game
	srand(time(0));
	int player = 1;
